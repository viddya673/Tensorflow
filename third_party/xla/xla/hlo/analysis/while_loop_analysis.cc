...
optional<Range> MatchTrivialLoopRange(const HloInstruction* while_op) {
  ...
  // return Range(indvar_init_val.value(), while_cond_bound_val.value(),
  //                       trip_count_step);
...
optional<int64_t> MatchTrivialLoopTripCount(const HloInstruction* while_op,
  ...
  if (Match(while_cond_root,
    ...
    VLOG(2) << "Pattern-match succeeded: loop condition is i < N: "
            << while_cond_root->ToString();
    optional<int64_t> trips = CheckedSubtract(range.max().GetUnsignedValue(),
                                              range.min().GetUnsignedValue());
    if (trips) {
      const int64_t remainder =
          std::remainder(*trips, range.step().GetUnsignedValue());
      const int64_t div = std::floor(*trips / range.step().GetUnsignedValue());
      if (remainder == 0) {
      ...
      if (*trips < range.max().GetUnsignedValue()) {
  ...
  if (Match(while_cond_root,
    ...
    trips = CheckedAdd(std::floor(*trips / range.step().GetUnsignedValue()), 1);
    if (!trips) {
      VLOG(2) << "Pattern-match failed: Trip count exceeds INT64_MAX";
      return nullopt;
    }
