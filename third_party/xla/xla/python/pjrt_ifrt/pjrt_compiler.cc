...
absl::Status TranslateDeviceIds(PjRtClient* client,
  ...
  auto translate_device_id =
      [&](DeviceId ifrt_device_id) -> absl::StatusOr<xla::PjRtGlobalDeviceId> {
    auto device = client->LookupDevice(ifrt_device_id);
    if (device.ok()) {
      xla::PjRtDevice* pjrt_device =
          tensorflow::down_cast<PjRtDevice*>(*device)->pjrt_device();
      if (pjrt_device == nullptr) {
        return Unimplemented(
            "Not supported to use non-addressable IFRT device ID %d",
            ifrt_device_id.value());
      } else {
        return pjrt_device->global_device_id();
      }
    } else {
      return InvalidArgument("Invalid IFRT device ID %d",
                             ifrt_device_id.value());
    }
  };
  ...
  if (options.executable_build_options.has_device_assignment()) {
    ...
    device_assignment.Each([&](int64_t replica, int64_t computation,
                               int64_t* device_id) {
      if (!result.ok()) {
        return;
      }
      auto pjrt_global_device_id = translate_device_id(DeviceId(*device_id));
