...
OpCostCalculator MakeOpCostCalculatorFromMap(
    absl::flat_hash_map<CostMetricId, CostValue> metric_map) {
  return
      [metric_map = std::move(metric_map)](const HloInstruction& instruction) {
        return [metric_map](const CostMetricId& metric_id) {
          auto it = metric_map.find(metric_id);
...
std::unique_ptr<OpCostManager::CalculationNode> MakeTestLeaf(
    const HloInstruction& instruction, bool enable_cache,
    OpCostManager::CalculationNode::AcceptCostFn accept_cost_fn = nullptr) {
  return OpCostManager::CalculationNode::CreateLeaf(
      "leaf-calculator",
      MakeOpCostCalculatorFromMap({
      ...
      enable_cache, std::move(accept_cost_fn));
}

// Implements the test for the LeafNoCache and LeafWithCache tests.
void RunLeafNodeCacheTest(const HloInstruction& instruction,
                          bool enable_cache) {
  std::unique_ptr<OpCostManager::CalculationNode> node =
      MakeTestLeaf(instruction, enable_cache);
...
// Tests that we can pass a value map to the leaf calculator and the correct
// results is stored.
TEST_F(OpCostTest, LeafNodeWithValueMap) {
  std::unique_ptr<OpCostManager::CalculationNode> node =
      MakeTestLeaf(*result_, /*enable_cache=*/false);
...
// Tests use of an accept cost function with a leaf node.
TEST_F(OpCostTest, LeafNodeWithAcceptFn) {
  std::unique_ptr<OpCostManager::CalculationNode> node = MakeTestLeaf(
      *result_, /*enable_cache=*/false,
      [](const CostMetricId& metric_id, double cost) { return cost != 100.0; });
...
//   * latency: 400
//   * else: not found
std::unique_ptr<OpCostManager::CalculationNode> MakeTestDelegationNode(
...
TEST_F(OpCostTest, DelegationNodeDefaultOrder) {
  std::unique_ptr<OpCostManager::CalculationNode> node =
      MakeTestDelegationNode(*result_);
...
TEST_F(OpCostTest, DelegationNodeCustomDelegationFn) {
  ...
  std::unique_ptr<OpCostManager::CalculationNode> node =
      MakeTestDelegationNode(*result_, std::move(delegation_order_fn));
...
TEST_F(OpCostTest, DelegationNodeDifferentOrdersForDifferentInstructions) {
  ...
  children.push_back(OpCostManager::CalculationNode::CreateLeaf(
      "leaf0",
      [](const HloInstruction& instruction) {
        MetricCalculator metric_calculator = [](const CostMetricId& metric_id) {
          return CostValue::MakeValue(100.0);
        };
  ...
  children.push_back(OpCostManager::CalculationNode::CreateLeaf(
      ...
      [](const HloInstruction& instruction) {
        MetricCalculator metric_calculator = [](const CostMetricId& metric_id) {
          return CostValue::MakeValue(200.0);
        };
...
void RunOpCostManagerTest(const HloInstruction& instruction, bool enable_cache,
  ...
  EXPECT_EQ(op_cost_manager.OperandBytesAccessed(instruction, /*operand_num=*/0,
                                                 ShapeIndex({1})),
            20.0);
  EXPECT_EQ(
...
void RunHloCostAnalysisWithAcceptStateTest(const HloModule& module,
  ...
  if (accepted_entry_computation) {
    EXPECT_OK(module.entry_computation()->Accept(
        hlo_cost_analysis_for_wrapper.get()));
  }
  HloCostAnalysisWithAcceptState hlo_cost_analysis_wrapper(
      std::move(hlo_cost_analysis_for_wrapper), accepted_entry_computation);

  HloCostAnalysis hlo_cost_analysis;
  EXPECT_OK(module.entry_computation()->Accept(&hlo_cost_analysis));

  // It should be ok to keep getting cost analysis results, without crashing for
  // calling Accept() multiple times.
  for (int i = 0; i < 10; ++i) {
    EXPECT_EQ(hlo_cost_analysis_wrapper.cost_analysis(add).flop_count(add),
              hlo_cost_analysis.flop_count(add));
  }
}

TEST_F(OpCostTest, HloCostAnalysisWithAcceptStateNotAcceptedYet) {
  RunHloCostAnalysisWithAcceptStateTest(*module_, *add0_,
                                        /*accepted_entry_computation=*/false);
}

TEST_F(OpCostTest, HloCostAnalysisWithAcceptStateAlreadyAccepted) {
  RunHloCostAnalysisWithAcceptStateTest(*module_, *add0_,
                                        /*accepted_entry_computation=*/true);
}
...
TEST_F(OpCostTest, CalculatorWithDefaultTotalBytesAccessed) {
  ...
  EXPECT_EQ((op_cost_calculator(*result_))(
                CostMetricId::TotalBytesAccessed(*result_)),
            CostValue::MakeValue(2000.0));

  OpCostCalculator op_cost_calculator_with_default_total_bytes_accessed =
      CreateCalculatorWithDefaultTotalBytesAccessed(
          std::move(op_cost_calculator));
  EXPECT_EQ((op_cost_calculator_with_default_total_bytes_accessed(*result_))(
                CostMetricId::TotalBytesAccessed(*result_)),
...
TEST_F(OpCostTest, CalculatorWithPostProcessedValues) {
  ...
  EXPECT_EQ(
      (op_cost_calculator(*result_))(CostMetricId::LatencySeconds(*result_)),
      CostValue::MakeValue(16.0));
  EXPECT_EQ(
      (op_cost_calculator(*result_))(CostMetricId::ComputeSeconds(*result_)),
      CostValue::MakeValue(4.0));
}
