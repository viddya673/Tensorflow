...
absl::Status RunCollectiveOptimizationPasses(
  ...
  if (debug_options.xla_gpu_enable_pipelined_collectives() ||
      ...
      config.exec_time_optimization_effort() >= 0.2) {
...
void AddDoubleBufferingPasses(const HloModuleConfig& config,
                              HloPassPipeline& pipeline) {
  const DebugOptions& opts = config.debug_options();
...
absl::Status RunPostFusionPasses(
  ...
  AddDoubleBufferingPasses(hlo_module->config(), pipeline);
