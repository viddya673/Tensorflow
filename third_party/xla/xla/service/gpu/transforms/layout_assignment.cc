...
absl::Status GpuLayoutAssignment::SetDotOperandLayoutToMinorContracting(
  ...
  const bool contracting_dims_are_minor =
      absl::c_all_of(contracting_dims,
                     [&, dim = batch_dims.size() + noncontracting_dims.size()](
                         int64_t i) mutable { return i = dim++; });

  // If contracting dims are already minor, and the layout is valid, keep it.
  if (contracting_dims_are_minor && shape.has_layout() &&
